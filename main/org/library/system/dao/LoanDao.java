...
public class LoanDao {
    ...
    public int create(Loan loan) throws SQLException {
        ...
        try (Connection connection = ConectionDB.getConnection();
            ...
            statement.setDate(4, Date.valueOf(loan.getDue_date()));
    ...
    public boolean update(Loan loan) throws SQLException {
        ...
        try (Connection connection = ConectionDB.getConnection();
            ...
            statement.setDate(3, Date.valueOf(loan.getDue_date()));
    ...
    private Loan mapLoan(ResultSet rs) throws SQLException {
        ...
        loan.setRequest_id(rs.getInt("request_id"));
        loan.setStudent_id(rs.getInt("student_id"));
        loan.setLibrarian_id(rs.getInt("librarian_id"));

        Date loanDate = rs.getDate("loan_date");
        if (loanDate != null) {
            loan.setLoan_date(loanDate.toLocalDate());
        }

        Date dueDate = rs.getDate("due_date");
        if (dueDate != null) {
            loan.setDue_date(dueDate.toLocalDate());
        }
